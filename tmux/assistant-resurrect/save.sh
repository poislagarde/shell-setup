...
codex_session_id() {
	local sid start db state_file="$STATE_DIR/codex-$1.json"

	# 1. State file from the Codex SessionStart hook — exact, per-process, like
	#    claude method 1. Codex's hook session_id is the UUID `codex resume`
	#    takes, so trust it directly. No-ops when the hook isn't installed/firing
	#    (file absent) — methods 2-3 still apply.
	if [ -f "$state_file" ]; then
		sid=$(jq -r '.session_id // empty' "$state_file" 2>/dev/null || true)
		if [ -n "$sid" ] && ! id_used "$sid"; then
			echo "$sid"
	...
	# 2. `resume <uuid>` in the process args (freshly restored codex).
	...
	# 3. Codex's own state DB: ~/.codex/state_*.sqlite (filename version-
	...
	#    assigned to another pane. Opened with `?immutable=1`, NOT -readonly:
	#    the running Codex app-server holds the db and blocks a plain readonly
	#    connection (CANTOPEN), so immutable reads the file directly. (Trade-off:
	#    immutable ignores an active -wal, so a just-created thread not yet
	#    checkpointed may be momentarily invisible — the hook path 1 covers that.)
	db=$(ls -t "$HOME"/.codex/state_*.sqlite 2>/dev/null | head -1)
	[ -n "$db" ] || return 0
	start=$(process_start_epoch "$1")
	# Escape the cwd for SQL by doubling single quotes (the SQL-standard escape).
	# Build it in a variable: substituting inline in the query string keeps the
	# backslashes (…o\'\'brien…), which SQLite rejects ("unrecognized token").
	local sq=\' cwd_esc
	cwd_esc=${3//$sq/$sq$sq}
	while read -r sid; do
		if [ -n "$sid" ] && ! id_used "$sid"; then
	...
	done < <(sqlite3 "file:$db?immutable=1" \
		"SELECT id FROM threads WHERE cwd = '$cwd_esc' AND archived = 0 \
